...
{
    public class BackgroundObjectArea : AreaConfigBase
    {
        [Header("적용 범위")]
...
#pragma warning disable 0414
        ...
        string alert =
@"해당 영역 설정은 외부 모듈인 Example의 기능을 사용합니다.
씬에 Example 모듈이 등록되어있는지 확인하세요.";
#pragma warning restore 0414
        ...
        public override void Validate(List<string> warnings)
        {
            ...
            {
                if (Resolver?.Example == null) warnings.Add($"{GetType().Name}: 현재 씬에 Example 모듈이 등록되지 않았습니다.");
            }
        }
        ...
        {
            var area = Resolver?.Example?.GetArea();
            if (area == null) return;
            center = area.Value.center;
            size = area.Value.size;
        }
    }
}