...
{
    public class CameraFixedSizeZoomStrategy : CameraZoomStrategy
    {
        FixedSizeZoomActionConfig _fixedConfig;

        public CameraFixedSizeZoomStrategy(FixedSizeZoomActionConfig config) : base(config)
        {
            _fixedConfig = config;
        }
        ...
        public override void Calculate(InputContext context, Camera camera, CameraState currentState)
        {
            ...
            var amount = _fixedConfig.fixedSize;
            if (context.Delta < 0) amount *= -1f;
